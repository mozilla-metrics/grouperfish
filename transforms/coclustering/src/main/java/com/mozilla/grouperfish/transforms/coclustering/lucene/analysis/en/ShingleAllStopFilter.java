...
public final class ShingleAllStopFilter extends TokenFilter {
     ...
     */
    public ShingleAllStopFilter(TokenStream input, int minShingleSize, int maxShingleSize, Set<?> stopwords) {
        super(input);
     ...
     *            maximum shingle size produced by the filter.
     */
    public ShingleAllStopFilter(TokenStream input, int maxShingleSize) {
        this(input, DEFAULT_MIN_SHINGLE_SIZE, maxShingleSize, StandardAnalyzer.STOP_WORDS_SET);
    }
     ...
     * @param stopwords
     */
    public ShingleAllStopFilter(TokenStream input, int maxShingleSize, Set<?> stopwords) {
        this(input, DEFAULT_MIN_SHINGLE_SIZE, maxShingleSize, stopwords);
    }
     ...
     *            input stream
     */
    public ShingleAllStopFilter(TokenStream input) {
        this(input, DEFAULT_MIN_SHINGLE_SIZE, DEFAULT_MAX_SHINGLE_SIZE, StandardAnalyzer.STOP_WORDS_SET);
    }
     ...
     */
    public ShingleAllStopFilter(TokenStream input, String tokenType) {
        this(input, DEFAULT_MIN_SHINGLE_SIZE, DEFAULT_MAX_SHINGLE_SIZE, StandardAnalyzer.STOP_WORDS_SET);
        ...
        if (inputWindow.size() >= gramSize.getValue()) {
            ...
            for (int gramNum = 1; iter.hasNext() && builtGramSize < gramSize.getValue(); ++gramNum) {
                ...
                if (builtGramSize < gramNum) {
                    ...
                    String term = new String(nextToken.termAtt.buffer(), 0, nextToken.termAtt.length());
                    if (!stopwords.contains(term)) {
                        isAllStopwords = false;
                    }
                    gramBuilder.append(term);
                    ++builtGramSize;
                }
     ...
     * <code>positionIncrement - 1</code> {@link #FILLER_TOKEN}s are inserted
     * first.
     *
     * @param target
    ...
    private InputWindowToken getNextToken(InputWindowToken target) throws IOException {
        ...
        if (numFillerTokensToInsert > 0) {
            ...
            if (posIncrAtt.getPositionIncrement() > 1) {
                ...
                // so no more than (maxShingleSize - 1) filler tokens will be
                // inserted.

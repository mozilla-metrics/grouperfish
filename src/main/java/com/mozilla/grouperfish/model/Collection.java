 ...
 * 
 * With collections, the opposite is the case, because we usually want to
 * get/update very specific datums. Updates to other columns make no sense and
 * might even undo other valid updates. When put to the db, only the given
 * columns will be overwritten. Set an attributes to <tt>null</tt> to prevent it
 * from being written to the database.
 */
public class Collection implements Model {

	@Override
	public CollectionRef ref() {
		return ref_;
	}

	public Long get(Attribute attr) {
		return attributes_.get(attr);
	}

	public Collection set(Attribute attr, Long value) {

...
import com.mozilla.grouperfish.input.Opinions.Field;
...
/**
 * Loads opinions from a stream into all applicable collections. Also creates
 * these collections.
 */
public class StreamImporter {

	public StreamImporter(final Conf conf, final String namespace, final List<String> keyPatterns) {
	...
	private static enum Counters {
		ROWS_USED, ROWS_DISCARDED, DOCS_CREATED, COLLECTIONS_CREATED
	};
	...
	class DocumentsGenerator implements Iterable<Document> {
		...
		DocumentsGenerator(final InputStream in, final Map<String, CollectionRef> collectionRefs,
		...
		public Iterator<Document> iterator() {
			return new Iterator<Document>() {
				...
				public Document next() {
					...
					final DocumentRef ref = new DocumentRef(collectionRefs_.get(key), opinion_[Field.ID.i]);
				...
				public boolean hasNext() {
					...
					do
						opinion_ = it_.next();
					while (it_.hasNext() && !isOk(opinion_));
	 ...
	 * field names like variables. Examples: Note that there is no escape
	 * syntax. "$PRODUCT:$VERSION-$PLATFORM-$TYPE" will produce keys such as
	...
	private static class KeyGen {
		...
		String key(final String[] opinion) {
			...
			for (Field field : fields_) {
				if (field == null)
					sb.append(fragments_[frag++]);
				else
					sb.append(opinion[field.ordinal()]);
			}
			return sb.toString();
		}

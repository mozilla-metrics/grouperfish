...
import com.mozilla.grouperfish.hbase.keys.Keys;
import com.mozilla.grouperfish.hbase.keys.ReversePartsKeys;
import com.mozilla.grouperfish.hbase.keys.SimpleKeys;
...
public class Factory {
	...
	public Factory(Conf conf) {
		...
		if ("REVERSE_PARTS".equals(keyScheme))
			keys_ = new ReversePartsKeys();
		else if ("SIMPLE".equals(keyScheme))
			keys_ = new SimpleKeys();
		else
			keys_ = Assert.unreachable(Keys.class, "Unknown key scheme: %s", keyScheme);

		hbaseConf_ = HBaseConfiguration.create();
		final String zkQuorum = conf_.get(CONF_ZK_QUORUM);
		if (zkQuorum != null)
			hbaseConf_.set("hbase.zookeeper.quorum", zkQuorum);
		final String zkZnode = conf_.get(CONF_ZK_ZNODE);
		if (zkZnode != null)
			hbaseConf_.set("zookeeper.znode.parent", zkZnode);
	}
	...
	public String tableName(final Class<?> model) {

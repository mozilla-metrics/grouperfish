...
public class ReversePartsKeys extends Keys {
	...
	protected String document(String ns, String ck, String docID) {
		...
		return revAppend(new StringBuilder(size).append(ns).append('/').append(ck).append('/'), docID).toString();
	}
	...
	protected String cluster(String ns, String ck, long rebuildTS, String label) {
		...
		final int size = "DEFAULT/".length() + ns.length() + 1 + ck.length() + 1 + ts.length() + 1 + label.length();

		return revAppend(new StringBuilder(size).append("DEFAULT/").append(ns).append('/').append(ck).append('/'), ts)
